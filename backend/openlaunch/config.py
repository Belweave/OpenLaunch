...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = AlembicConfig(OPENLAUNCH_DIR / 'alembic.ini')

        migrations_path = OPENLAUNCH_DIR / 'migrations'
...
STATIC_DIR = Path(os.getenv('STATIC_DIR', OPENLAUNCH_DIR / 'static')).resolve()
...
if CUSTOM_NAME:
    log.warning('CUSTOM_NAME is deprecated; hosted custom-brand lookup is not available in OpenLaunch.')
...
if ENV == 'prod':
    if OLLAMA_BASE_URL == '/ollama' and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == 'true':
            # if you use all-in-one docker container (OpenLaunch + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = 'http://localhost:11434'
        else:
            OLLAMA_BASE_URL = 'http://host.docker.internal:11434'
    elif K8S_FLAG:
        OLLAMA_BASE_URL = 'http://ollama-service.openlaunch.svc.cluster.local:11434'
...
MILVUS_COLLECTION_PREFIX = os.getenv('MILVUS_COLLECTION_PREFIX', 'openlaunch')
...
QDRANT_COLLECTION_PREFIX = os.getenv('QDRANT_COLLECTION_PREFIX', 'openlaunch')
...
ELASTICSEARCH_INDEX_PREFIX = os.getenv('ELASTICSEARCH_INDEX_PREFIX', 'openlaunch_collections')
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv('PINECONE_INDEX_NAME', 'openlaunch-index')
...
VALKEY_COLLECTION_PREFIX = os.getenv('VALKEY_COLLECTION_PREFIX', 'openlaunch')
...
OPENLAUNCH_URL = os.getenv('OPENLAUNCH_URL', '')


ENABLE_SIGNUP = False if not OPENLAUNCH_AUTH else os.getenv('ENABLE_SIGNUP', 'True').lower() == 'true'
...
USER_PERMISSIONS_FEATURES_SQL_DATABASE = (
    os.getenv('USER_PERMISSIONS_FEATURES_SQL_DATABASE', 'False').lower() == 'true'
)

USER_PERMISSIONS_FEATURES_DATA_SOURCES = (
    os.getenv(
        'USER_PERMISSIONS_FEATURES_DATA_SOURCES',
        str(USER_PERMISSIONS_FEATURES_SQL_DATABASE),
    ).lower()
    == 'true'
)
...
DEFAULT_USER_PERMISSIONS = {
    ...
    'features': {
        ...
        'sql_database': USER_PERMISSIONS_FEATURES_SQL_DATABASE,
        'data_sources': USER_PERMISSIONS_FEATURES_DATA_SOURCES,
...
try:
    banners = json.loads(os.getenv('OPENLAUNCH_BANNERS', '[]'))
    banners = [BannerModel(**banner) for banner in banners]
except Exception as e:
    log.exception(f'Error loading OPENLAUNCH_BANNERS: {e}')
    banners = []

OPENLAUNCH_BANNERS = banners
