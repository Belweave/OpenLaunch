...
from openlaunch.env import (
    ...
    OPENLAUNCH_AUTH,
    OPENLAUNCH_AUTH_COOKIE_SAME_SITE,
    OPENLAUNCH_AUTH_COOKIE_SECURE,
    OPENLAUNCH_AUTH_SIGNOUT_REDIRECT_URL,
    OPENLAUNCH_AUTH_TRUSTED_EMAIL_HEADER,
    OPENLAUNCH_AUTH_TRUSTED_GROUPS_HEADER,
    OPENLAUNCH_AUTH_TRUSTED_NAME_HEADER,
    OPENLAUNCH_AUTH_TRUSTED_ROLE_HEADER,
)
from openlaunch.internal.db import get_async_session
from openlaunch.models.auths import (
    AddUserForm,
    ApiKey,
    Auths,
    LdapForm,
...
from openlaunch.models.users import (
    UpdateProfileForm,
    UserModel,
...
from openlaunch.utils.access_control import get_permissions, has_permission
...
async def create_session_response(
    ...
    if set_cookie and response:
        ...
        response.set_cookie(
            ...
            httponly=True,
            samesite=OPENLAUNCH_AUTH_COOKIE_SAME_SITE,
            secure=OPENLAUNCH_AUTH_COOKIE_SECURE,
...
async def update_password(
    ...
    if OPENLAUNCH_AUTH_TRUSTED_EMAIL_HEADER:
...
async def signin(
    ...
    if OPENLAUNCH_AUTH_TRUSTED_EMAIL_HEADER:
        ...
        if OPENLAUNCH_AUTH_TRUSTED_EMAIL_HEADER not in request.headers:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, detail=ERROR_MESSAGES.INVALID_TRUSTED_HEADER)

        email = request.headers[OPENLAUNCH_AUTH_TRUSTED_EMAIL_HEADER].lower()
        name = email

        if OPENLAUNCH_AUTH_TRUSTED_NAME_HEADER:
            name = request.headers.get(OPENLAUNCH_AUTH_TRUSTED_NAME_HEADER, email)
            try:
        ...
        user = await Auths.authenticate_user_by_email(email, db=db)
        if user:
            if OPENLAUNCH_AUTH_TRUSTED_GROUPS_HEADER:
                group_names = request.headers.get(OPENLAUNCH_AUTH_TRUSTED_GROUPS_HEADER, '').split(',')
                group_names = [name.strip() for name in group_names if name.strip()]

                if group_names:
                    await Groups.sync_groups_by_group_names(user.id, group_names, db=db)

            if OPENLAUNCH_AUTH_TRUSTED_ROLE_HEADER:
                trusted_role = request.headers.get(OPENLAUNCH_AUTH_TRUSTED_ROLE_HEADER, '').lower().strip()
...
async def signout(request: Request, response: Response, db: AsyncSession = Depends(get_async_session)):
                            ...
                            if logout_url:
                                return JSONResponse(
                                    ...
                                    content={
                                        ...
                                        + (
                                            f'&post_logout_redirect_uri={OPENLAUNCH_AUTH_SIGNOUT_REDIRECT_URL}'
                                            if OPENLAUNCH_AUTH_SIGNOUT_REDIRECT_URL
                                            else ''
                                        ),
                                    },
                                    headers=response.headers,
    ...
    if OPENLAUNCH_AUTH_SIGNOUT_REDIRECT_URL:
        return JSONResponse(
            ...
            content={
                'status': True,
                'redirect_url': OPENLAUNCH_AUTH_SIGNOUT_REDIRECT_URL,
            },
            headers=response.headers,
        )
...
async def token_exchange(
    ...
    """
    Exchange an external OAuth provider token for an OpenLaunch JWT.
