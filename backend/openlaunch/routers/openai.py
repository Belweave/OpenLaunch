...
from openlaunch.utils.access_control import check_model_access, has_connection_access, has_permission
from openlaunch.utils.anthropic import get_anthropic_models, is_anthropic_url
...
from openlaunch.utils.payload import (
    apply_model_params_to_body_openai,
    apply_system_prompt_to_body,
)
from openlaunch.utils.session_pool import (
    cleanup_response,
    get_session,
    stream_wrapper,
)
...
async def get_headers_and_cookies(
    ...
    headers = {
        ...
        **(
            {
                'HTTP-Referer': 'https://github.com/belweave/openlaunch',
                'X-Title': 'OpenLaunch',
            }
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'OpenLaunch: Server Connection Error',
            )
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail='OpenLaunch: Server Connection Error')
            except Exception as e:
...
def _normalize_stored_item(item: dict) -> dict:
    ...
    OpenLaunch stores extra bookkeeping fields (``id``, ``status``,
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=r.status if r else 500,
            detail='OpenLaunch: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r)
