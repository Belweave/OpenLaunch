...
from openlaunch.internal.db import Base, get_async_db
...
LEGACY_CONFIG_KEY_ALIASES = {'openlaunch.url': ''.join(('web', 'ui.url'))}
API_CONFIG_FIELDS = (
    'enable',
    'key',
    'prefix_id',
...
class Config(Base):
    ...
    async def get(key: str, default: Any = None) -> Any:
        ...
        async with get_async_db() as db:
            ...
            if row is None and key in LEGACY_CONFIG_KEY_ALIASES:
                row = await db.get(Config, LEGACY_CONFIG_KEY_ALIASES[key])
    ...
    async def get_many(*keys: str) -> dict:
        ...
        lookup_keys = enabled_keys | {
            LEGACY_CONFIG_KEY_ALIASES[key] for key in enabled_keys if key in LEGACY_CONFIG_KEY_ALIASES
        }
        async with get_async_db() as db:
            result = await db.execute(select(Config).where(Config.key.in_(lookup_keys)))
            values = {row.key: row.value for row in result.scalars().all()}
            return {
                key: values.get(
                    key,
                    values.get(LEGACY_CONFIG_KEY_ALIASES.get(key, ''), Config.default_value(key)),
                )
                for key in keys
                if key in values
                or LEGACY_CONFIG_KEY_ALIASES.get(key) in values
                or key in Config.DEFAULTS
                or key in disabled_values
            }
