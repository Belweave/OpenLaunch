...
from openlaunch.constants import TASKS
...
from openlaunch.models.folders import Folders
from openlaunch.models.functions import Functions
from openlaunch.models.models import Models
from openlaunch.models.oauth_sessions import OAuthSessions
from openlaunch.models.users import UserModel, Users
from openlaunch.retrieval.utils import get_sources_from_items
...
from openlaunch.routers.pipelines import (
...
from openlaunch.routers.tasks import (
    generate_chat_tags,
    generate_follow_ups,
...
from openlaunch.utils.access_control import has_connection_access, has_permission
from openlaunch.utils.access_control.files import get_accessible_folder_files
from openlaunch.utils.chat import generate_chat_completion
from openlaunch.utils.code_interpreter import execute_code_jupyter
from openlaunch.utils.context_compaction import compact_messages_for_request
from openlaunch.utils.files import (
    convert_markdown_base64_images,
    get_file_url_from_base64,
    get_image_base64_from_url,
    get_image_url_from_base64,
)
from openlaunch.utils.filter import (
    get_sorted_filter_ids,
    process_filter_functions,
)

from openlaunch.utils.mcp.client import MCPClient
from openlaunch.utils.memory import add_memory_context, review_memory_after_turn
...
from openlaunch.utils.payload import apply_system_prompt_to_body, resolve_system_prompt
from openlaunch.utils.plugin import load_function_module_by_id
from openlaunch.utils.response import merge_usage, normalize_usage
from openlaunch.utils.sanitize import sanitize_code
from openlaunch.utils.task import (
    get_task_model_id,
    rag_template,
    tools_function_calling_generation_template,
)
from openlaunch.utils.tools import (
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if skill_ids:
        from openlaunch.models.skills import Skills as SkillsModel
                ...
                if content or response_output:
                    ...
                    if await Config.get('ui.enable_user_webhooks') and not await Users.is_user_active(user.id):
                        ...
                        if webhook_url:
                            ...
                            await post_webhook(
                                request.app.state.OPENLAUNCH_NAME,
                                webhook_url,
                                f'{content}\n\n{title} - {openlaunch_url}/c/{metadata["chat_id"]}',
                                {
                                    'action': 'chat',
                                    'message': content,
                                    'title': title,
                                    'url': f'{openlaunch_url}/c/{metadata["chat_id"]}',
                                },
                            )
        ...
        async def response_handler(response, events):
            def tag_output_handler(content_type, tags, output):
                ...
                output_type_map = {
                    ...
                    'code_interpreter': 'openlaunch:code_interpreter',
                }
                ...
                if last_type == 'message':
                    ...
                    for start_tag, end_tag in tags:
                        ...
                        if match:
                            ...
                            elif output_item_type == 'openlaunch:code_interpreter':
                                output.append(
                                    {
                                        'type': 'openlaunch:code_interpreter',
                                        'id': output_id('ci'),
                                        'status': 'in_progress',
                                        'start_tag': start_tag,
                                        'end_tag': end_tag,
                    ...
                    or (last_type == 'openlaunch:code_interpreter' and content_type == 'code_interpreter')
                    ...
                    elif last_type == 'openlaunch:code_interpreter':
            ...
            try:
                                    ...
                                    if value:
                                        ...
                                        inside_tag_block = (
                                            ...
                                            and (
                                                ...
                                                or last_item_type == 'openlaunch:code_interpreter'
                                                or (
                                        ...
                                        if inside_tag_block:
                                            ...
                                            if last_item_type == 'openlaunch:code_interpreter':
                ...
                if DETECT_CODE_INTERPRETER:
                    ...
                    while output and output[-1].get('type') == 'openlaunch:code_interpreter' and retries < MAX_RETRIES:
                        await event_emitter(
                            {
                                'type': 'chat:completion',
                                'data': {
