...
from openlaunch.internal.db import get_async_db
from openlaunch.models.automations import AutomationModel, AutomationRuns, Automations
...
from openlaunch.utils.auth import create_token
from openlaunch.utils.misc import parse_duration
from openlaunch.utils.task import prompt_template
...
async def _set_terminal_cwd(app, server_id: str, user, cwd: str, chat_id: str) -> None:
    ...
    Routes through the openlaunch terminal proxy endpoint so that
    auth headers, orchestrator policy routing, and X-User-Id are
    handled correctly — same path the frontend uses.
    """
    import aiohttp
    from openlaunch.env import AIOHTTP_CLIENT_SESSION_SSL
...
async def execute_automation(app, automation: AutomationModel) -> None:
    ...
    try:
        ...
        from openlaunch.socket.main import sio
...
async def _check_calendar_alerts(app) -> None:
    ...
    """
    from openlaunch.models.calendar import CalendarEvents, CalendarEventUpdateForm
    from openlaunch.socket.main import sio
    ...
    for event, user_tz in upcoming:
        ...
        try:
            openlaunch_name = getattr(app.state, 'OPENLAUNCH_NAME', 'OpenLaunch')
