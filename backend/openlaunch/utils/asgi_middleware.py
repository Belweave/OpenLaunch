...
In OpenLaunch this surfaces as:
...
from openlaunch.env import CUSTOM_API_KEY_HEADER
from openlaunch.internal.db import ScopedSession
from openlaunch.models.config import Config
from openlaunch.utils.auth import get_http_authorization_cred
...
class AuthTokenMiddleware:
    ...
    This is useful when OpenLaunch sits behind a reverse proxy that
    consumes the ``Authorization`` header for its own authentication —
    set the env var to a unique header (e.g. ``X-OpenLaunch-Key``) so
        ...
        if 'shared' in query_params and query_params['shared']:
            ...
            if text:
                ...
                if url_match:
                    ...
                    # YouTube URL.
                    from openlaunch.retrieval.loaders.youtube import _parse_video_id
