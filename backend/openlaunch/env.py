...
# Promote legacy branded environment variables before configuration is read.
# The names are assembled so the deprecated brand never appears in runtime
# output, documentation, or canonical configuration.
_LEGACY_ENV_PREFIX = ''.join(('WEB', 'UI_'))
for _key, _value in list(os.environ.items()):
    if _key.startswith(_LEGACY_ENV_PREFIX):
        os.environ.setdefault(f'OPENLAUNCH_{_key.removeprefix(_LEGACY_ENV_PREFIX)}', _value)
...
# OPENLAUNCH_DIR should be the directory where env.py resides (openlaunch/)
OPENLAUNCH_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of OPENLAUNCH_DIR (backend/)
BACKEND_DIR = OPENLAUNCH_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (openlaunch-dev/)
...
if FROM_INIT_PY:
    PACKAGE_DATA = {'version': importlib.metadata.version('openlaunch')}
else:
    try:
...
try:
    ...
    changelog_content = (pkgutil.get_data('openlaunch', 'CHANGELOG.md') or b'').decode()
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv('DATA_DIR', OPENLAUNCH_DIR / 'data')).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / 'openlaunch_data', 'zip', DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv('DATA_DIR', OPENLAUNCH_DIR / 'data'))

STATIC_DIR = Path(os.getenv('STATIC_DIR', OPENLAUNCH_DIR / 'static'))

FONTS_DIR = Path(os.getenv('FONTS_DIR', OPENLAUNCH_DIR / 'static' / 'fonts'))

FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', BASE_DIR / 'build')).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', OPENLAUNCH_DIR / 'frontend')).resolve()

####################################
# Database
####################################

_canonical_database_path = DATA_DIR / 'openlaunch.db'
_legacy_database_path = DATA_DIR / ''.join(('web', 'ui.db'))
for _database_path in (DATA_DIR / 'ollama.db', _legacy_database_path):
    if _database_path.exists() and not _canonical_database_path.exists():
        os.rename(_database_path, _canonical_database_path)
        log.info('Existing database migrated to the OpenLaunch filename successfully.')
        break

DATABASE_URL = os.getenv('DATABASE_URL', f'sqlite:///{DATA_DIR}/openlaunch.db')
...
if all(DB_VARS.values()):
    ...
    DATABASE_URL = f'sqlite+sqlcipher:///{DATA_DIR}/openlaunch.db'
...
REDIS_KEY_PREFIX = os.getenv('REDIS_KEY_PREFIX', 'openlaunch')
...
# but is scoped to OpenLaunch to avoid interfering with system-level settings.
...
def _parse_ssl_env(value: str) -> 'bool | _ssl.SSLContext':
    ...
    This allows users with corporate or internal CAs to point OpenLaunch
...
OPENLAUNCH_AUTH = os.getenv('OPENLAUNCH_AUTH', 'True').lower() == 'true'
...
# OPENLAUNCH_JWT_SECRET_KEY is deprecated; use OPENLAUNCH_SECRET_KEY instead.
# No hardcoded fallback by design: the supported start scripts set/auto-generate it; unset is rejected below.
OPENLAUNCH_SECRET_KEY = os.getenv(
    'OPENLAUNCH_SECRET_KEY',
    os.getenv('OPENLAUNCH_JWT_SECRET_KEY', ''),
)

ENABLE_VALVE_ENCRYPTION = os.getenv('ENABLE_VALVE_ENCRYPTION', 'False').lower() == 'true'

OPENLAUNCH_SESSION_COOKIE_SAME_SITE = os.getenv('OPENLAUNCH_SESSION_COOKIE_SAME_SITE', 'lax')
OPENLAUNCH_SESSION_COOKIE_SECURE = os.getenv('OPENLAUNCH_SESSION_COOKIE_SECURE', 'false').lower() == 'true'
OPENLAUNCH_AUTH_COOKIE_SAME_SITE = os.getenv('OPENLAUNCH_AUTH_COOKIE_SAME_SITE', OPENLAUNCH_SESSION_COOKIE_SAME_SITE)
OPENLAUNCH_AUTH_COOKIE_SECURE = (
    os.getenv(
        'OPENLAUNCH_AUTH_COOKIE_SECURE',
        os.getenv('OPENLAUNCH_SESSION_COOKIE_SECURE', 'false'),
    ).lower()
    == 'true'
)

if OPENLAUNCH_AUTH and OPENLAUNCH_SECRET_KEY == '':
    raise SystemExit(
        'OPENLAUNCH_SECRET_KEY is not set. It is a hard requirement when authentication is enabled.\n'
        'The supported start methods set or auto-generate it for you: use start.sh (Linux/macOS), '
        'start_windows.bat (Windows), or `openlaunch serve`.\n'
        'If you start the backend another way (e.g. invoking uvicorn directly, which is unsupported), '
        'you must set OPENLAUNCH_SECRET_KEY yourself to a long random value.\n'
...
OPENLAUNCH_ADMIN_EMAIL = os.getenv('OPENLAUNCH_ADMIN_EMAIL', '')
OPENLAUNCH_ADMIN_PASSWORD = os.getenv('OPENLAUNCH_ADMIN_PASSWORD', '')
OPENLAUNCH_ADMIN_NAME = os.getenv('OPENLAUNCH_ADMIN_NAME', 'Admin')

OPENLAUNCH_AUTH_TRUSTED_EMAIL_HEADER = os.getenv('OPENLAUNCH_AUTH_TRUSTED_EMAIL_HEADER', None)
OPENLAUNCH_AUTH_TRUSTED_NAME_HEADER = os.getenv('OPENLAUNCH_AUTH_TRUSTED_NAME_HEADER', None)
OPENLAUNCH_AUTH_TRUSTED_GROUPS_HEADER = os.getenv('OPENLAUNCH_AUTH_TRUSTED_GROUPS_HEADER', None)
OPENLAUNCH_AUTH_TRUSTED_ROLE_HEADER = os.getenv('OPENLAUNCH_AUTH_TRUSTED_ROLE_HEADER', None)

# Custom header name for API key authentication.  Defaults to 'x-api-key'.
# Useful when OpenLaunch sits behind a reverse proxy / API gateway that
# already uses the Authorization header for its own authentication — set
# this to a unique header (e.g. 'X-OpenLaunch-Key') so the middleware
...
# compatible APIs for endpoints not natively handled by OpenLaunch.
ENABLE_OPENAI_API_PASSTHROUGH = os.getenv('ENABLE_OPENAI_API_PASSTHROUGH', 'False').lower() == 'true'

OPENLAUNCH_AUTH_SIGNOUT_REDIRECT_URL = os.getenv('OPENLAUNCH_AUTH_SIGNOUT_REDIRECT_URL', None)
...
OAUTH_CLIENT_INFO_ENCRYPTION_KEY = os.getenv('OAUTH_CLIENT_INFO_ENCRYPTION_KEY', OPENLAUNCH_SECRET_KEY)

OAUTH_SESSION_TOKEN_ENCRYPTION_KEY = os.getenv('OAUTH_SESSION_TOKEN_ENCRYPTION_KEY', OPENLAUNCH_SECRET_KEY)
...
# Allows external apps to exchange OAuth tokens for OpenLaunch tokens
...
OPENLAUNCH_NAME = os.getenv('OPENLAUNCH_NAME', 'OpenLaunch')
...
FORWARD_USER_INFO_HEADER_USER_NAME = os.getenv('FORWARD_USER_INFO_HEADER_USER_NAME', 'X-OpenLaunch-User-Name')
FORWARD_USER_INFO_HEADER_USER_ID = os.getenv('FORWARD_USER_INFO_HEADER_USER_ID', 'X-OpenLaunch-User-Id')
FORWARD_USER_INFO_HEADER_USER_EMAIL = os.getenv('FORWARD_USER_INFO_HEADER_USER_EMAIL', 'X-OpenLaunch-User-Email')
FORWARD_USER_INFO_HEADER_USER_ROLE = os.getenv('FORWARD_USER_INFO_HEADER_USER_ROLE', 'X-OpenLaunch-User-Role')
FORWARD_SESSION_INFO_HEADER_MESSAGE_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_MESSAGE_ID', 'X-OpenLaunch-Message-Id')
FORWARD_SESSION_INFO_HEADER_CHAT_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_CHAT_ID', 'X-OpenLaunch-Chat-Id')

# If set while ENABLE_FORWARD_USER_INFO_HEADERS is True, send one signed HS256 JWT
# (FORWARD_USER_INFO_HEADER_JWT) instead of separate X-OpenLaunch-User-* headers.
FORWARD_USER_INFO_HEADER_JWT_SECRET = (os.environ.get('FORWARD_USER_INFO_HEADER_JWT_SECRET') or '').strip() or None
FORWARD_USER_INFO_HEADER_JWT = os.environ.get('FORWARD_USER_INFO_HEADER_JWT', 'X-OpenLaunch-User-Jwt')
try:
...
OTEL_SERVICE_NAME = os.getenv('OTEL_SERVICE_NAME', 'openlaunch')
