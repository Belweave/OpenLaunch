...
from openlaunch.constants import ERROR_MESSAGES, TASKS
from openlaunch.env import (
    ...
    OPENLAUNCH_ADMIN_EMAIL,
    OPENLAUNCH_ADMIN_NAME,
    OPENLAUNCH_ADMIN_PASSWORD,
    OPENLAUNCH_AUTH_TRUSTED_EMAIL_HEADER,
    OPENLAUNCH_BUILD_HASH,
    OPENLAUNCH_SECRET_KEY,
    OPENLAUNCH_SESSION_COOKIE_SAME_SITE,
    OPENLAUNCH_SESSION_COOKIE_SECURE,
)
from openlaunch.events import (
    EVENTS,
...
from openlaunch.internal.db import engine, get_async_session
from openlaunch.models.access_grants import AccessGrants
from openlaunch.models.channels import Channels
from openlaunch.models.chats import ChatForm, Chats
from openlaunch.models.config import Config
from openlaunch.models.functions import Functions
from openlaunch.models.messages import Messages
...
from openlaunch.routers.retrieval import (
    get_ef,
    get_embedding_function,
    get_reranking_function,
    get_rf,
)
from openlaunch.socket.main import (
    MODELS,
    get_event_emitter,
    get_models_in_use,
...
from openlaunch.utils import logger
from openlaunch.utils.access_control import has_permission
from openlaunch.utils.actions import chat_action as chat_action_handler
from openlaunch.utils.asgi_middleware import (
    AuthTokenMiddleware,
    CommitSessionMiddleware,
    RedirectMiddleware,
    WebsocketUpgradeGuardMiddleware,
)
from openlaunch.utils.audit import AuditLevel, AuditLoggingMiddleware
from openlaunch.utils.auth import (
    create_admin_user,
    decode_token,
    get_admin_user,
...
from openlaunch.utils.embeddings import generate_embeddings
from openlaunch.utils.logger import start_logger
from openlaunch.utils.middleware import (
    background_tasks_handler,
    build_chat_response_context,
    process_chat_payload,
    process_chat_response,
)
from openlaunch.utils.models import (
    check_model_access,
    get_all_base_models,
    get_all_models,
    get_filtered_models,
)
from openlaunch.utils.oauth import (
...
from openlaunch.utils.plugin import install_tool_and_function_dependencies
from openlaunch.utils.redis import get_redis_client
from openlaunch.utils.security_headers import SecurityHeadersMiddleware
from openlaunch.utils.session_pool import get_session
from openlaunch.utils.tools import set_terminal_servers, set_tool_servers
...
if LOG_FORMAT != 'json':
    banner = (
        f'OpenLaunch v{VERSION} - self-hosted AI interface.\n'
        + (f'Commit: {OPENLAUNCH_BUILD_HASH}\n' if OPENLAUNCH_BUILD_HASH != 'dev-build' else '')
    ...
    try:
        ...
        print(f'OpenLaunch v{VERSION} - building the best AI user interface.\nhttps://github.com/belweave/openlaunch')
...
async def lifespan(app: FastAPI):
    ...
    if OPENLAUNCH_ADMIN_EMAIL and OPENLAUNCH_ADMIN_PASSWORD:
        if await create_admin_user(OPENLAUNCH_ADMIN_EMAIL, OPENLAUNCH_ADMIN_PASSWORD, OPENLAUNCH_ADMIN_NAME):
    ...
    from openlaunch.utils.automations import scheduler_worker_loop
    ...
    from openlaunch.utils.session_pool import close_session
...
if ENABLE_OTEL:
    from openlaunch.utils.telemetry.setup import setup as setup_opentelemetry
...
# `openlaunch.utils.asgi_middleware` for the rationale.
...
async def generate_messages(
    ...
    All models configured in OpenLaunch are accessible via this endpoint.
    ...
    return {
        **({'onboarding': True} if onboarding else {}),
        ...
        'features': {
            ...
            'auth': OPENLAUNCH_AUTH,
            'auth_trusted_header': bool(OPENLAUNCH_AUTH_TRUSTED_EMAIL_HEADER),
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for OpenLaunch.
...
try:
    if ENABLE_STAR_SESSIONS_MIDDLEWARE:
        ...
        app.add_middleware(
            ...
            cookie_same_site=OPENLAUNCH_SESSION_COOKIE_SAME_SITE,
            cookie_https_only=OPENLAUNCH_SESSION_COOKIE_SECURE,
        )
    ...
    app.add_middleware(
        SessionMiddleware,
        secret_key=OPENLAUNCH_SECRET_KEY,
        session_cookie='owui-session',
        same_site=OPENLAUNCH_SESSION_COOKIE_SAME_SITE,
        https_only=OPENLAUNCH_SESSION_COOKIE_SECURE,
    )
...
async def get_manifest_json():
    ...
    if external_pwa_manifest_url:
        ...
        return {
            ...
            'description': f'{app.state.OPENLAUNCH_NAME} is an open, extensible, user-friendly interface for AI that adapts to your workflow.',
            'start_url': '/',
            'display': 'standalone',
