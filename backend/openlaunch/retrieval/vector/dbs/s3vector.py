...
import boto3
from openlaunch.config import S3_VECTOR_BUCKET_NAME, S3_VECTOR_REGION
from openlaunch.retrieval.vector.main import (
    GetResult,
    SearchResult,
    VectorDBBase,
    VectorItem,
)
from openlaunch.retrieval.vector.utils import process_metadata
...
class S3VectorClient(VectorDBBase):
    """
    AWS S3 Vector integration for OpenLaunch Knowledge.
    """
    ...
    def _filter_metadata(self, metadata: Dict[str, Any], item_id: str) -> Dict[str, Any]:
        ...
        # Keep only the first 10 keys, prioritizing important ones based on actual OpenLaunch metadata
        important_keys = [
    ...
    def get(self, collection_name: str) -> Optional[GetResult]:
        ...
        try:
            ...
            # The OpenLaunch GetResult expects lists of lists, so we wrap each list
            if all_ids:
