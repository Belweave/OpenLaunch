...
OPENLAUNCH_LATEST_RELEASE_API_URL = (
...
async def get_app_latest_release_version(user=Depends(get_verified_user)):
    if OFFLINE_MODE:
        ...
        return {
            "current": VERSION,
            "latest": VERSION,
            "release_url": OPENLAUNCH_RELEASES_URL,
        }
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                ...
                if response.status == 404:
                    log.debug("No stable OpenLaunch release is available yet")
                    return {
                        "current": VERSION,
                        "latest": VERSION,
                        "release_url": OPENLAUNCH_RELEASES_URL,
                ...
                return {
                    ...
                    "release_url": data.get("html_url", OPENLAUNCH_RELEASES_URL),
                }
    except Exception as e:
        log.debug(e)
        return {
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            ...
            "description": "OpenLaunch is an open, extensible, user-friendly interface for AI that adapts to your workflow.",
            "start_url": "/",
            "display": "standalone",
...
async def get_opensearch_xml():
    ...
    <Image width="16" height="16" type="image/x-icon">{app.state.config.WEBUI_URL}/static/openlaunch_logo.png</Image>
