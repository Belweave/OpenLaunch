...
import { getAnthropicModelsDirect } from './anthropic';
			...
			for (const idx in OPENAI_API_BASE_URLS) {
				...
				if (enable) {
					if (modelIds.length > 0) {
						...
						requests.push(
							(provider === 'anthropic'
								? getAnthropicModelsDirect(url, OPENAI_API_KEYS[idx])
								: getOpenAIModelsDirect(url, OPENAI_API_KEYS[idx])
							).catch(() => ({ object: 'list', data: [], urlIdx: idx }))
						);
					}
				} else {
					requests.push(Promise.resolve({ object: 'list', data: [], urlIdx: idx }));
				}
			}
