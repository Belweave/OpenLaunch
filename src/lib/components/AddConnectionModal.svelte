<script lang="ts">
	...
	const verifyHandler = () => {
		if (ollama) {
			verifyOllamaHandler();
		} else if (direct && provider === 'anthropic') {
			verifyAnthropicConnection(url, key)
				.then(() => toast.success($i18n.t('Server connection verified')))
					...
					<div class="px-1">
						{#if direct && !ollama}
							<div class="flex flex-col w-full mb-2">
								<div class="mb-0.5 text-xs text-gray-500">{$i18n.t('API Provider')}</div>
								<select
									...
									on:change={() => {
										if (
											!url ||
											['https://api.openai.com/v1', 'https://api.anthropic.com'].includes(url)
										) {
											url =
												provider === 'anthropic'
							...
							{:else}
								<div class="text-gray-500 text-xs text-center py-2 px-10">
									...
									{:else if provider === 'anthropic'}
										{$i18n.t('Leave empty to include all models from the Anthropic Models API')}
									{:else}
