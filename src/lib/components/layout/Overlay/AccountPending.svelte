			...
			<div class="max-w-md">
				...
				>
					...
					{:else}
						...
						{$i18n.t('Contact Admin for OpenLaunch Access')}
					{/if}
				</div>
				...
				>
					...
					{:else}
						{$i18n.t('Your account status is currently pending activation.')}{'\n'}{$i18n.t(
							'To access the OpenLaunch, please reach out to the administrator. Admins can manage user statuses from the Admin Panel.'
						)}
					{/if}
				</div>
