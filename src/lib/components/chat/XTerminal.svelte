<script lang="ts">
	...
	const getTerminalInfo = (): { serverId: string; baseUrl: string } | null => {
		...
		if (systemMatch) {
			// For system terminals, WS goes through the OpenLaunch backend proxy
			return { serverId: systemMatch.id, baseUrl: OPENLAUNCH_API_BASE_URL };
		}
	...
	const connect = async () => {
		...
		try {
			...
			if (info.serverId === '__direct__') {
				...
				// System terminal — proxy through OpenLaunch backend
				const base = info.baseUrl.replace(/\/$/, '');
				authToken = token;
