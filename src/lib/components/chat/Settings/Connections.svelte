				...
				<div class="pr-1.5">
					...
					<div class="my-1.5">
						<div class="text-xs text-gray-500">
							{$i18n.t(
								'Connect directly to OpenAI-compatible endpoints or the native Anthropic Messages API.'
							)}
							<br />
							{$i18n.t(
								'CORS must be properly configured by the provider to allow requests from OpenLaunch.'
							)}
							<br />
							{$i18n.t(
								'API keys are stored in your user settings and sent directly from this browser.'
							)}
						</div>
					</div>
				</div>
