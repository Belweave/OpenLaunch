<script lang="ts">
	...
	import { OPENLAUNCH_NAME, config, showChangelog } from '$lib/stores';
	...
	const checkForVersionUpdates = async () => {
		...
		version = await getVersionUpdates(localStorage.token).catch((error) => {
			return {
				current: OPENLAUNCH_VERSION,
				latest: OPENLAUNCH_VERSION
			};
		});
			...
			<div class="flex w-full justify-between items-center">
				<div class="flex flex-col text-xs text-gray-700 dark:text-gray-200">
					<div class="flex gap-1">
						<Tooltip content={OPENLAUNCH_BUILD_HASH}>
							v{OPENLAUNCH_VERSION}
						</Tooltip>

						{#if $config?.features?.enable_version_update_check}
							<a
								href="https://github.com/belweave/openlaunch/releases/tag/v{version.latest}"
								target="_blank"
							>
								{updateAvailable === null
		...
		<div>
			<pre
				class="text-xs text-gray-400 dark:text-gray-500">Copyright (c) {new Date().getFullYear()} <a
					...
					target="_blank"
					class="underline">Belweave and OpenLaunch contributors</a
				> · <a href="https://github.com/belweave/openlaunch/blob/main/LICENSE" target="_blank"
					>MIT License</a
				>
</pre>
		</div>
