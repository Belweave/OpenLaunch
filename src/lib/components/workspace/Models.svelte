<script lang="ts">
	...
	import { OPENLAUNCH_NAME, config, mobile, models as _models, settings, user } from '$lib/stores';
	...
	const shareModelHandler = async (model) => {
		toast.success($i18n.t('Redirecting you to OpenLaunch Community'));

		const url = 'https://github.com/belweave/openlaunch';

		const tab = await window.open(url, '_blank');
							...
							<div class="flex group/item gap-3.5 w-full">
								<div class="self-center pl-0.5">
									<div class="flex bg-white rounded-2xl">
										...
										>
											<img
												src={`${OPENLAUNCH_API_BASE_URL}/models/model/profile/image?id=${model.id}&lang=${$i18n.language}`}
	...
	{#if $config?.features.enable_community_sharing}
		<div class=" my-16">
			<div class=" text-xl font-medium mb-1 line-clamp-1">
				{$i18n.t('Made by OpenLaunch Community')}
			</div>
