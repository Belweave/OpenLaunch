<script>
	...
	import { anthropicChatCompletion } from '$lib/apis/anthropic';
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${title} | OpenLaunch`, {
								body: content,
								icon: `${WEBUI_BASE_URL}/static/openlaunch_logo.png`
							});
						}
					}
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isFocused) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${data?.user?.name} (#${event?.channel?.name}) | OpenLaunch`, {
							body: data?.content,
							icon: data?.user?.profile_image_url ?? `${WEBUI_BASE_URL}/static/openlaunch_logo.png`
						});
					}
				}
...
<svelte:head>
	...
	<link crossorigin="anonymous" rel="icon" href="{WEBUI_BASE_URL}/static/openlaunch_logo.png" />
