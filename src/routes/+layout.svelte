<script>
	...
	const setupSocket = async (enableWebsocket) => {
		const _socket = io(`${OPENLAUNCH_BASE_URL}` || undefined, {
			reconnection: true,
		...
		_socket.on('connect', async () => {
			...
			if (version !== null || deploymentId !== null) {
				if (
					($OPENLAUNCH_VERSION !== null && version !== $OPENLAUNCH_VERSION) ||
					($OPENLAUNCH_DEPLOYMENT_ID !== null && deploymentId !== $OPENLAUNCH_DEPLOYMENT_ID)
				) {
					await unregisterServiceWorkers();
			...
			if (deploymentId !== null) {
				OPENLAUNCH_DEPLOYMENT_ID.set(deploymentId);
			}

			if (version !== null) {
				OPENLAUNCH_VERSION.set(version);
				window.OPENLAUNCH_VERSION = version;
			}
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isInBackground) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${title} • OpenLaunch`, {
							body: data?.content,
							icon: `${OPENLAUNCH_API_BASE_URL}/users/${data?.user?.id}/profile/image`
						});
					}
				}
	...
	const isAuthenticatedBackendFetch = (input, init) => {
		try {
			...
			const backendOrigin = new URL(OPENLAUNCH_BASE_URL || '/', window.location.origin).origin;
	...
	const isCurrentSessionUnauthorized = async (originalFetch) => {
		return originalFetch(`${OPENLAUNCH_API_BASE_URL}/auths/`, {
			method: 'GET',
			headers: {
	...
	onMount(async () => {
		...
		if (backendConfig) {
			...
			await OPENLAUNCH_NAME.set(backendConfig.name);
