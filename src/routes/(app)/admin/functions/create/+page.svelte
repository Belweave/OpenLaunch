<script>
	...
	import { OPENLAUNCH_VERSION } from '$lib/constants';
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_openlaunch_version ?? '0.0.0', OPENLAUNCH_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'OpenLaunch version (v{{OPENLAUNCH_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						OPENLAUNCH_VERSION: OPENLAUNCH_VERSION,
						REQUIRED_VERSION: manifest?.required_openlaunch_version ?? '0.0.0'
					}
				)
			);
			return;
