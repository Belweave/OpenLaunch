...
# ---------------------------------------------------------------------------
# Interactive docker compose launcher for OpenLaunch.
...
Options:
  ...
  --openlaunch[port=PORT]      Set the OpenLaunch port (default: 3000)
...
Examples:
  ...
  $(basename "$0") --enable-gpu[count=all] --openlaunch[port=8080]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --openlaunch*)       openlaunch_port=$(extract_bracket_value "$1" "3000") ;;
...
export OPENLAUNCH_PORT="$openlaunch_port"
...
echo -e "   ${GREEN}${BOLD}OpenLaunch Port:${RESET}  ${openlaunch_port}"
